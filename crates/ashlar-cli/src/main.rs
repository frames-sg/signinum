    ...
    match subcommand.as_deref() {
        Some("inspect") => {
            let path = match args.next() {
                Some(p) => p,
                None => {
                    eprintln!("usage: ashlar inspect <file>");
        ...
        Some("--help") | Some("-h") | Some("help") | None => {
            eprintln!("ashlar {}", env!("CARGO_PKG_VERSION"));
            eprintln!("Usage:");
            eprintln!("  ashlar inspect <file>    Parse JPEG or JPEG 2000 headers and print Info");
            ExitCode::SUCCESS
        }
        Some(other) => {
...
fn inspect_bytes(bytes: &[u8]) -> Result<String, String> {
    match detect_inspect_format(bytes) {
        InspectFormat::Jpeg => match ashlar_jpeg::Decoder::inspect(bytes) {
            Ok(info) => Ok(format!(
            ...
            Err(e) => {
                ...
                if e.is_unsupported() {
                    message.push_str(
                        "\nhint: this file is not supported by ashlar; try jpeg-decoder or openjpeg",
                    );
                }
                Err(message)
            }
        },
        InspectFormat::J2k => match ashlar_j2k::J2kDecoder::inspect(bytes) {
            Ok(info) => Ok(format!(
