...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=ASHLAR_GROK_ROOT");
    println!("cargo:rerun-if-env-changed=ASHLAR_GROK_SOURCE");
...
fn grok_config() -> Option<GrokConfig> {
    let source_root = std::env::var_os("ASHLAR_GROK_SOURCE")
        .map_or_else(|| PathBuf::from("/tmp/grok-ashlar"), PathBuf::from);
    let lib_dir = std::env::var_os("ASHLAR_GROK_ROOT")
