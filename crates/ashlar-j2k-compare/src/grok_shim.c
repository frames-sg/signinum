...
static void ashlar_grok_init_once(void) {
...
static uint8_t ashlar_clamp_u8(int32_t value) {
  if (value < 0) {
    return 0;
  }
  if (value > 255) {
...
int ashlar_grok_decode_u8(const uint8_t *bytes, size_t len, uint32_t reduce,
  ...
  *out_height = 0;
  ...
  for (uint32_t row = 0; row < height; ++row) {
    for (uint32_t col = 0; col < width; ++col) {
      ...
      if (channels == 1) {
        packed[dst] = ashlar_clamp_u8(v0);
        continue;
      }
      if (image->numcomps == 1) {
        uint8_t gray = ashlar_clamp_u8(v0);
        packed[dst] = gray;
        packed[dst + 1] = gray;
        packed[dst + 2] = gray;
        continue;
      ...
      packed[dst] = ashlar_clamp_u8(v0);
      packed[dst + 1] = ashlar_clamp_u8(v1);
      packed[dst + 2] = ashlar_clamp_u8(v2);
    }
  }
...
void ashlar_grok_free(void *ptr) { free(ptr); }
