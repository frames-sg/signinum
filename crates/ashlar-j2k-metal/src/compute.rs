...
use ashlar_core::{PixelFormat, Rect};
#[cfg(target_os = "macos")]
use ashlar_j2k_native::HtCodeBlockDecoder;
...
impl HtCodeBlockDecoder for MetalCodeBlockDecoder {
    ...
    fn decode_j2k_code_block(
        &mut self,
        job: ashlar_j2k_native::J2kCodeBlockDecodeJob<'_>,
        output: &mut [f32],
    ) -> ashlar_j2k_native::Result<bool> {
    ...
    fn decode_inverse_mct(&mut self, job: J2kInverseMctJob<'_>) -> ashlar_j2k_native::Result<bool> {
...
fn prepare_classic_sub_band(
    job: &ashlar_j2k_native::J2kOwnedSubBandPlan,
    ...
    for block in &job.jobs {
        ...
        jobs.push(J2kClassicCleanupBatchJob {
            ...
            sub_band_type: match block.sub_band_type {
                ashlar_j2k_native::J2kSubBandType::LowLow => 0,
                ashlar_j2k_native::J2kSubBandType::HighLow => 1,
                ashlar_j2k_native::J2kSubBandType::LowHigh => 2,
                ashlar_j2k_native::J2kSubBandType::HighHigh => 3,
            },
...
fn prepare_ht_sub_band(
    job: &ashlar_j2k_native::HtOwnedSubBandPlan,
...
fn lookup_direct_band_slice(
    ...
    band_id: J2kDirectBandId,
    rect: ashlar_j2k_native::J2kRect,
...
#[cfg(target_os = "macos")]
fn rect_shapes_match(first: ashlar_j2k_native::J2kRect, other: ashlar_j2k_native::J2kRect) -> bool {
...
#[cfg(target_os = "macos")]
fn classic_style_flags(style: ashlar_j2k_native::J2kCodeBlockStyle) -> u32 {
    let mut flags = 0u32;
    ...
    fn prepared_ht_direct_plan_groups_cleanup_subbands_before_idwt() {
        ...
        let ht_subband_steps = plan
            ...
            .filter(|step| {
                matches!(
                    step,
                    ashlar_j2k_native::J2kDirectGrayscaleStep::HtSubBand(_)
                )
            })
            .count();
        assert!(
    ...
    fn prepared_classic_direct_plan_groups_cleanup_subbands_before_idwt() {
        ...
        let classic_subband_steps = plan
            ...
            .filter(|step| {
                matches!(
                    step,
                    ashlar_j2k_native::J2kDirectGrayscaleStep::ClassicSubBand(_)
                )
            })
            .count();
        assert!(
...
pub(crate) fn decode_image_to_surface<'a>(
    ...
    with_runtime(|runtime| {
        ...
        let decoded = image
            ...
            .map_err(|error| Error::Decode(ashlar_j2k::J2kError::Backend(error.to_string())))?;
