...
#[cfg(target_os = "macos")]
use ashlar_j2k::J2kError;
#[cfg(target_os = "macos")]
use ashlar_j2k_native::{DecoderContext as NativeDecoderContext, Image as NativeImage};
        ...
        match step {
            ...
            J2kDirectGrayscaleStep::Idwt(idwt) => {
                ...
                match idwt.transform {
                    ashlar_j2k_native::J2kWaveletTransform::Reversible53 => {
                        compute::decode_reversible53_single_decomposition_idwt(job, &mut output)?;
                    }
                    ashlar_j2k_native::J2kWaveletTransform::Irreversible97 => {
            ...
            J2kDirectGrayscaleStep::Store(store) => {
                ...
                let job = ashlar_j2k_native::J2kStoreComponentJob {
                    input,
...
fn decode_classic_sub_band(plan: &J2kOwnedSubBandPlan, output: &mut [f32]) -> Result<(), Error> {
    ...
    compute::decode_classic_cleanup_sub_band(
        ashlar_j2k_native::J2kSubBandDecodeJob {
            width: plan.width,
            height: plan.height,
            jobs: &jobs,
        },
...
fn decode_ht_sub_band(plan: &HtOwnedSubBandPlan, output: &mut [f32]) -> Result<(), Error> {
    ...
    compute::decode_ht_cleanup_sub_band(
        ashlar_j2k_native::HtSubBandDecodeJob {
            width: plan.width,
            height: plan.height,
            jobs: &jobs,
        },
...
mod tests {
    ...
    fn classic_plan() -> ashlar_j2k_native::J2kDirectGrayscalePlan {
    ...
    fn ht_plan() -> ashlar_j2k_native::J2kDirectGrayscalePlan {
        let pixels: Vec<u8> = (0..16).collect();
        let options = EncodeOptions {
            reversible: true,
            num_decomposition_levels: 1,
        ...
        for step in &plan.steps {
            match step {
                ...
                J2kDirectGrayscaleStep::Idwt(idwt) => {
                    ...
                    let mut output =
                        vec![0.0f32; idwt.rect.width() as usize * idwt.rect.height() as usize];
                    let job = ashlar_j2k_native::J2kSingleDecompositionIdwtJob {
                        rect: idwt.rect,
                        transform: idwt.transform,
                        ll: ashlar_j2k_native::J2kIdwtBand {
                            rect: idwt.ll,
                            coefficients: ll,
                        },
                        hl: ashlar_j2k_native::J2kIdwtBand {
                            rect: idwt.hl,
                            coefficients: hl,
                        },
                        lh: ashlar_j2k_native::J2kIdwtBand {
                            rect: idwt.lh,
                            coefficients: lh,
                        },
                        hh: ashlar_j2k_native::J2kIdwtBand {
                            rect: idwt.hh,
                            coefficients: hh,
                        },
                    };
    ...
    fn execute_to_first_idwt_inputs(
        ...
        ashlar_j2k_native::J2kDirectIdwtStep,
        Vec<f32>,
        Vec<f32>,
        Vec<f32>,
        Vec<f32>,
    ...
    struct CaptureIdwtJob {
        rect: Option<ashlar_j2k_native::J2kRect>,
        ll_rect: Option<ashlar_j2k_native::J2kRect>,
        hl_rect: Option<ashlar_j2k_native::J2kRect>,
        lh_rect: Option<ashlar_j2k_native::J2kRect>,
        hh_rect: Option<ashlar_j2k_native::J2kRect>,
        ll: Vec<f32>,
        hl: Vec<f32>,
        lh: Vec<f32>,
        hh: Vec<f32>,
    ...
    impl HtCodeBlockDecoder for CaptureIdwtJob {
        fn decode_code_block(
            &mut self,
            job: ashlar_j2k_native::HtCodeBlockDecodeJob<'_>,
            output: &mut [f32],
        ) -> ashlar_j2k_native::Result<()> {
            ashlar_j2k_native::decode_ht_code_block_scalar(job, output)
        }
