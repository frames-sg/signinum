/*!
Internal pure-Rust JPEG 2000 codec engine for `ashlar`.

This module tree was imported from the `dicom-toolkit-jpeg2000` 0.5.0 crate
and adapted in-repo so `ashlar-j2k` no longer depends on an external
...
# Example
```rust,no_run
use ashlar_j2k_native::{DecodeSettings, Image};
