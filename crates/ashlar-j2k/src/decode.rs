...
use ashlar_core::{BufferError, DecodeOutcome, Downscale, PixelFormat, Rect, Unsupported};
...
fn decode_image_into(
    ...
    let mut native_context = ashlar_j2k_native::DecoderContext::default();
...
pub(crate) fn decode_image_into_with_native_context<'a>(
    image: &Image<'a>,
    native_context: &mut ashlar_j2k_native::DecoderContext<'a>,
    out: &mut [u8],
    stride: usize,
    fmt: PixelFormat,
) -> Result<(), J2kError> {
    ...
    match fmt {
        ...
        _ => Err(Unsupported {
            what: "pixel format is not yet supported by ashlar-j2k",
        }
        .into()),
    }
}
...
pub(crate) fn validate_supported_format(fmt: PixelFormat) -> Result<(), J2kError> {
    if matches!(fmt, PixelFormat::Rgba16) {
        return Err(Unsupported {
            what: "Rgba16 output is not supported by ashlar-j2k M1",
        }
        .into());
    }
    Ok(())
