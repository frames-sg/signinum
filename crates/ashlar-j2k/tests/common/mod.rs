...
use ashlar_core::{Downscale, PixelFormat, Rect};
...
pub(crate) fn ashlar_rgb(bytes: &[u8]) -> Vec<u8> {
...
pub(crate) fn ashlar_rgb_region(bytes: &[u8], roi: Rect) -> Vec<u8> {
    let mut decoder = J2kDecoder::new(bytes).expect("decoder");
...
pub(crate) fn ashlar_rgb_scaled_q4(bytes: &[u8]) -> Vec<u8> {
    let mut decoder = J2kDecoder::new(bytes).expect("decoder");
    let dims = decoder.info().dimensions;
...
fn openjpeg_compress_bin() -> Option<PathBuf> {
    ...
    OPENJPEG_COMPRESS
        .get_or_init(|| {
            if let Some(path) = std::env::var_os("ASHLAR_OPENJPEG_COMPRESS_BIN") {
...
fn openjpeg_temp_dir() -> PathBuf {
    ...
    DIR.get_or_init(|| {
        let dir = std::env::temp_dir().join("ashlar-j2k-openjpeg-tests");
