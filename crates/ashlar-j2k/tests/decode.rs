...
use ashlar_j2k::{J2kCodec, J2kContext, J2kDecoder, J2kError};
use ashlar_j2k_native::{encode, encode_htj2k, DecodeSettings, EncodeOptions, Image};
...
fn backend_decode_u8_region(bytes: &[u8], roi: Rect) -> Vec<u8> {
    let mut context = ashlar_j2k_native::DecoderContext::default();
    Image::new(bytes, &DecodeSettings::default())
        .expect("backend image")
...
fn decode_rgb8_codestream_roundtrips_reversible_pixels() {
    ...
    let outcome = decoder
        ...
        .expect("decode");
    assert_eq!(outcome.decoded, ashlar_core::Rect::full((2, 2)));
