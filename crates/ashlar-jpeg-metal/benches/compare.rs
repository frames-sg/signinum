...
use ashlar_jpeg_metal::{Codec, Decoder, MetalSession, ScratchPool};
...
fn color_space_mode(color_space: ashlar_jpeg::ColorSpace) -> Option<DecodeMode> {
    match color_space {
        ashlar_jpeg::ColorSpace::Grayscale => Some(DecodeMode::Gray),
        ashlar_jpeg::ColorSpace::YCbCr | ashlar_jpeg::ColorSpace::Rgb => Some(DecodeMode::Rgb),
        ashlar_jpeg::ColorSpace::Cmyk | ashlar_jpeg::ColorSpace::Ycck => None,
    }
}
...
fn to_jpeg_rect(rect: Rect) -> ashlar_jpeg::Rect {
    ashlar_jpeg::Rect {
        x: rect.x,
        y: rect.y,
        w: rect.w,
...
fn cpu_decode_tile_batch(bytes: &[u8], batch_size: usize) {
    ...
    for _ in 0..batch_size {
        let decoder = CpuDecoder::from_view_in_context(
            ashlar_jpeg::JpegView::parse(bytes).expect("view"),
            &mut ctx,
        )
        .expect("decoder");
...
fn scheduled_viewport_surface(
    ...
    workload: &ashlar_jpeg_metal::viewport::ViewportWorkload,
