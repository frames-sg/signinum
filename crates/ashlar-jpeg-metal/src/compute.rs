...
use ashlar_core::{BackendRequest, PixelFormat, Rect};
use ashlar_jpeg::{
    adapter::{
...
impl ComponentRowWriter for PlaneStage {
    fn write_gray_row(&mut self, y: u32, gray_row: &[u8]) -> Result<(), ashlar_jpeg::JpegError> {
...
#[cfg(target_os = "macos")]
fn full_mcu_window_422(dims: (u32, u32), roi: ashlar_jpeg::Rect) -> ashlar_jpeg::Rect {
    let x0 = (roi.x / 16) * 16;
    let y0 = (roi.y / 8) * 8;
...
fn fast422_region_params(
    packet: &JpegMetalFast422PacketV1,
    fmt: PixelFormat,
    source_window: ashlar_jpeg::Rect,
) -> Result<JpegFast420Params, Error> {
    let out_format = pixel_format_to_out_format(fmt).ok_or_else(|| Error::MetalKernel {
        message: format!("unsupported JPEG Metal fast422 pixel format {fmt:?}"),
    })?;
...
fn fast422_scaled_params(
    packet: &JpegMetalFast422PacketV1,
    scale: ashlar_core::Downscale,
) -> Option<JpegFast420ScaledParams> {
    let scale_shift = match scale {
        ashlar_core::Downscale::Half => 1,
        ashlar_core::Downscale::Quarter => 2,
        ashlar_core::Downscale::Eighth => 3,
        _ => return None,
    };
...
#[cfg(target_os = "macos")]
fn full_mcu_window_420(dims: (u32, u32), roi: ashlar_jpeg::Rect) -> ashlar_jpeg::Rect {
    let x0 = (roi.x / 16) * 16;
    let y0 = (roi.y / 16) * 16;
    let x1 = (roi.x + roi.w).div_ceil(16) * 16;
    ...
    for request in requests {
        let batchable_op = match request.op {
            ...
            | batch::BatchOp::Scaled(
                ashlar_core::Downscale::Half
                | ashlar_core::Downscale::Quarter
                | ashlar_core::Downscale::Eighth,
            )
            | batch::BatchOp::RegionScaled {
                scale:
                    ashlar_core::Downscale::Half
...
#[cfg(target_os = "macos")]
fn core_rect_to_jpeg(rect: Rect) -> ashlar_jpeg::Rect {
    ashlar_jpeg::Rect {
        x: rect.x,
        y: rect.y,
        w: rect.w,
...
fn encode_fast420_region_batch_item(
    ...
    let local_roi = ashlar_jpeg::Rect {
...
fn encode_fast420_scaled_region_batch_item(
    ...
    let scaled_roi = ashlar_jpeg::Rect {
        x: scaled_roi.x,
        y: scaled_roi.y,
        w: scaled_roi.w,
        h: scaled_roi.h,
...
fn try_decode_fast444_region_scaled_rgb_batch_to_surfaces(
    ...
    let first_scaled_roi = ashlar_jpeg::Rect {
        x: first_scaled.x,
        y: first_scaled.y,
        w: first_scaled.w,
        h: first_scaled.h,
...
#[cfg(target_os = "macos")]
fn scaled_rect_covering(rect: Rect, scale: ashlar_core::Downscale) -> Rect {
...
pub(crate) fn decode_to_surface(
    decoder: &CpuDecoder<'_>,
    pool: &mut ashlar_jpeg::ScratchPool,
    fmt: PixelFormat,
...
mod tests {
    ...
    fn auto_batched_packets_skip_distinct_region_scaled_requests() {
        let packet =
            ashlar_jpeg::adapter::build_metal_fast420_packet(BASELINE_420_RESTART).expect("packet");
        let roi = Rect {
            x: 0,
            y: 0,
            w: 16,
        ...
        let requests = vec![
            batch::QueuedRequest::new(
                ...
                batch::BatchOp::RegionScaled {
                    roi,
                    scale: ashlar_core::Downscale::Quarter,
                },
                None,
                None,
                Some(packet.clone()),
    ...
    fn auto_batched_packets_allow_large_repeated_region_scaled_requests() {
        ...
        let packet =
            ashlar_jpeg::adapter::build_metal_fast420_packet(BASELINE_420).expect("packet");
        let roi = Rect {
            x: 0,
            y: 0,
            w: 16,
    ...
    fn fast420_packet_region_scaled_decode_matches_cpu_region_scaled_bytes() {
        ...
        let scale = ashlar_core::Downscale::Quarter;
        let (expected, _) = decoder
    ...
    fn fast422_packet_full_decode_matches_cpu_bytes() {
        ...
        let packet =
            ashlar_jpeg::adapter::build_metal_fast422_packet(BASELINE_422).expect("packet");
        let (expected, _) = decoder.decode(PixelFormat::Rgb8).expect("cpu full decode");
    ...
    fn fast422_packet_scaled_decode_matches_cpu_scaled_bytes() {
        ...
        let packet =
            ashlar_jpeg::adapter::build_metal_fast422_packet(BASELINE_422).expect("packet");
        for (scale, dims) in [
            (ashlar_core::Downscale::Half, (8, 4)),
            (ashlar_core::Downscale::Quarter, (4, 2)),
        ] {
            let (expected, _) = decoder
                .decode_scaled(PixelFormat::Rgb8, scale)
                .expect("cpu scaled");
    ...
    fn fast422_packet_region_scaled_decode_matches_cpu_region_scaled_bytes() {
        ...
        let scale = ashlar_core::Downscale::Half;
        let (expected, _) = decoder
            .decode_region_scaled(PixelFormat::Rgb8, roi, scale)
            .expect("cpu region scaled");
        let scaled_roi = ashlar_jpeg::Rect {
    ...
    fn fast444_packet_full_decode_matches_cpu_bytes() {
        ...
        let packet =
            ashlar_jpeg::adapter::build_metal_fast444_packet(BASELINE_444).expect("packet");
        let (expected, _) = decoder.decode(PixelFormat::Rgb8).expect("cpu full decode");
