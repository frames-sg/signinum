...
fn cpu_contiguous_viewport_region_matches_direct_decode() {
    let decoder = Decoder::new(BASELINE_420).expect("decoder");
    let mut pool = ScratchPool::new();
    let workload = ashlar_jpeg_metal::viewport::ViewportWorkload {
...
fn auto_viewport_surface_path_prefers_cpu_for_small_contiguous_workloads() {
    ...
    let expected = ashlar_jpeg_metal::viewport::decode_viewport_region_cpu_to_surface(
        &decoder,
        &mut direct_pool,
        &workload,
    )
...
fn non_macos_auto_viewport_surface_returns_cpu_surface() {
    ...
    assert_eq!(
        ashlar_core::DeviceSurface::backend_kind(&surface),
        ashlar_core::BackendKind::Cpu
    );
}
