...
pub(crate) fn ashlar_inspect(bytes: &[u8]) {
    let info = Decoder::inspect(bytes).expect("ashlar inspect");
...
pub(crate) fn ashlar_decode(bytes: &[u8], mode: DecodeMode) {
    ...
    let (out, _) = dec.decode(fmt).expect("ashlar decode");
...
/// decoder; fairness is preserved by keeping this group ashlar-only.
pub(crate) fn ashlar_decode_reused(
    dec: &Decoder<'_>,
    out: &mut [u8],
    stride: usize,
    fmt: PixelFormat,
...
pub(crate) fn ashlar_decode_with_scratch(
    dec: &Decoder<'_>,
...
pub(crate) fn ashlar_decode_rows(bytes: &[u8]) {
    let dec = Decoder::new(bytes).expect("ashlar decoder");
    let mut sink = NullSink;
    dec.decode_rows(&mut sink).expect("ashlar decode_rows");
}

pub(crate) fn ashlar_decode_tile_batch(bytes: &[u8], batch_size: usize) {
    let mut ctx = DecoderContext::new();
    let mut pool = ScratchPool::new();
    let mut sink = NullSink;
    for _ in 0..batch_size {
        Decoder::decode_tile(bytes, &mut ctx, &mut pool, &mut sink)
            .expect("ashlar decode_tile batch");
    }
}
...
pub(crate) fn ashlar_decode_tile_batch_scaled(bytes: &[u8], batch_size: usize, factor: Downscale) {
    let info = Decoder::inspect(bytes).expect("ashlar inspect");
    ...
    for _ in 0..batch_size {
        ...
        .expect("ashlar scaled tile batch");
    }
    std::hint::black_box(out);
}
...
pub(crate) fn ashlar_decode_tile_batch_region_scaled(
    ...
    for _ in 0..batch_size {
        ...
        .expect("ashlar region-scaled tile batch");
    }
    std::hint::black_box(out);
}

pub(crate) fn ashlar_decode_region(bytes: &[u8], side: u32) {
    let dec = Decoder::new(bytes).expect("ashlar decoder");
...
pub(crate) fn ashlar_decode_scaled(bytes: &[u8], factor: Downscale) {
    let dec = Decoder::new(bytes).expect("ashlar decoder");
    let (out, _) = dec
...
pub(crate) fn ashlar_decode_region_scaled(bytes: &[u8], side: u32, factor: Downscale) {
    let dec = Decoder::new(bytes).expect("ashlar decoder");
    let roi = centered_roi(dec.info().dimensions, side);
    let (out, _) = dec
        .decode_region_scaled(PixelFormat::Rgb8, roi, factor)
        .expect("ashlar scaled region decode");
    std::hint::black_box(out);
}
