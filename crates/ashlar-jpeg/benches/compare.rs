...
use ashlar_jpeg::{Decoder, Downscale, ScratchPool};
use common::{
    ashlar_decode, ashlar_decode_region, ashlar_decode_region_scaled, ashlar_decode_reused,
    ashlar_decode_rows, ashlar_decode_scaled, ashlar_decode_tile_batch,
    ashlar_decode_tile_batch_region_scaled, ashlar_decode_tile_batch_scaled,
    ashlar_decode_with_scratch, ashlar_inspect, centered_roi,
    ...
    zune_inspect, DecodeMode, TurboJpegDecoder,
};
...
fn bench_compare(c: &mut Criterion) {
    ...
    for input in inputs.iter().filter(|input| {
        ...
        decode_rgb.bench_function(format!("ashlar/{}", input.name), |b| {
            b.iter(|| ashlar_decode(&input.bytes, DecodeMode::Rgb));
        });
    ...
    for input in inputs.iter().filter(|input| {
        ...
        decode_gray.bench_function(format!("ashlar/{}", input.name), |b| {
            b.iter(|| ashlar_decode(&input.bytes, DecodeMode::Gray));
        });
    ...
    for input in inputs.iter().filter(|input| {
        ...
        let dec = Decoder::new(&input.bytes).expect("ashlar decoder (reused-setup)");
        let (fmt, stride, len) = output_geometry(&dec, DecodeMode::Rgb);
        let mut out = vec![0u8; len];
        decode_reused_rgb.bench_function(format!("ashlar_reused/{}", input.name), |b| {
            b.iter(|| ashlar_decode_reused(&dec, &mut out, stride, fmt));
        });
    }
    ...
    for input in inputs.iter().filter(|input| {
        ...
        let mut out = vec![0u8; len];
        decode_reused_gray.bench_function(format!("ashlar_reused/{}", input.name), |b| {
            b.iter(|| ashlar_decode_reused(&dec, &mut out, stride, fmt));
        });
    }
    ...
    for input in inputs.iter().filter(|input| {
        ...
        let dec = Decoder::new(&input.bytes).expect("ashlar decoder (scratch-setup)");
        let (fmt, stride, len) = output_geometry(&dec, DecodeMode::Rgb);
        let mut out = vec![0u8; len];
        let mut pool = ScratchPool::new();
        // Warm the pool once so iteration 1 pays zero allocation cost.
        ashlar_decode_with_scratch(&dec, &mut pool, &mut out, stride, fmt);
        decode_scratch_rgb.bench_function(format!("ashlar_scratch/{}", input.name), |b| {
            b.iter(|| ashlar_decode_with_scratch(&dec, &mut pool, &mut out, stride, fmt));
        });
    }
    ...
    for input in inputs.iter().filter(|input| {
        ...
        let mut pool = ScratchPool::new();
        ashlar_decode_with_scratch(&dec, &mut pool, &mut out, stride, fmt);
        decode_scratch_gray.bench_function(format!("ashlar_scratch/{}", input.name), |b| {
            b.iter(|| ashlar_decode_with_scratch(&dec, &mut pool, &mut out, stride, fmt));
        });
    }
    ...
    {
        decode_rows_rgb.bench_function(format!("ashlar/{}", input.name), |b| {
            b.iter(|| ashlar_decode_rows(&input.bytes));
        });
    }
    ...
    for input in inputs.iter().filter(|input| input.mode == DecodeMode::Rgb) {
        wsi_tile_batch_rgb.bench_function(format!("ashlar/{}", input.name), |b| {
            b.iter(|| ashlar_decode_tile_batch(&input.bytes, 64));
        });
        if libjpeg_turbo_available() {
            let mut turbo = TurboJpegDecoder::new().expect("libjpeg-turbo decoder");
    ...
    for input in inputs.iter().filter(|input| {
        ...
        wsi_region_rgb.bench_function(format!("ashlar/{}", input.name), |b| {
            b.iter(|| ashlar_decode_region(&input.bytes, 256));
        });
    ...
    for input in inputs.iter().filter(|input| {
        ...
        wsi_scaled_rgb_q4.bench_function(format!("ashlar/{}", input.name), |b| {
            b.iter(|| ashlar_decode_scaled(&input.bytes, Downscale::Quarter));
        });
    ...
    for input in inputs.iter().filter(|input| {
        ...
        wsi_scaled_rgb_q8.bench_function(format!("ashlar/{}", input.name), |b| {
            b.iter(|| ashlar_decode_scaled(&input.bytes, Downscale::Eighth));
        });
    ...
    for input in inputs.iter().filter(|input| {
        ...
        let roi = centered_roi(input.dimensions, 256);
        wsi_region_scaled_rgb_q4.bench_function(format!("ashlar/{}", input.name), |b| {
            b.iter(|| ashlar_decode_region_scaled(&input.bytes, 256, Downscale::Quarter));
        });
    ...
    for input in inputs.iter().filter(|input| {
        ...
        let roi = centered_roi(input.dimensions, 256);
        wsi_region_scaled_rgb_q8.bench_function(format!("ashlar/{}", input.name), |b| {
            b.iter(|| ashlar_decode_region_scaled(&input.bytes, 256, Downscale::Eighth));
        });
    ...
    for input in inputs.iter().filter(|input| input.mode == DecodeMode::Rgb) {
        wsi_tile_batch_scaled_rgb_q4.bench_function(format!("ashlar/{}", input.name), |b| {
            b.iter(|| {
                ashlar_decode_tile_batch_scaled(&input.bytes, 64, Downscale::Quarter);
            });
        });
    ...
    for input in inputs.iter().filter(|input| input.mode == DecodeMode::Rgb) {
        let roi = centered_roi(input.dimensions, 256);
        wsi_tile_batch_region_scaled_rgb_q4.bench_function(format!("ashlar/{}", input.name), |b| {
            b.iter(|| {
                ashlar_decode_tile_batch_region_scaled(&input.bytes, 64, 256, Downscale::Quarter);
            });
        });
