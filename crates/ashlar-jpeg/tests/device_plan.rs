...
use ashlar_jpeg::{ColorSpace, Decoder, Warning};
...
fn adapter_device_plan_exposes_scan_metadata() {
    ...
    let plan = ashlar_jpeg::adapter::build_device_plan(&decoder, 4).expect("device plan");
...
fn adapter_device_plan_checkpoint_cadence_handles_multi_mcu_inputs() {
    ...
    let cadence_zero =
        ashlar_jpeg::adapter::build_device_plan(&decoder, 0).expect("zero-cadence plan");
    let cadence_two =
        ashlar_jpeg::adapter::build_device_plan(&decoder, 2).expect("cadence-two plan");
...
fn adapter_device_plan_restart_checkpoints_capture_resume_state() {
    ...
    let plan = ashlar_jpeg::adapter::build_device_plan(&decoder, 2).expect("device plan");
...
fn adapter_device_plan_surfaces_missing_eoi_warning() {
    ...
    let decoder = Decoder::new(&bytes).expect("decoder");
    let plan = ashlar_jpeg::adapter::build_device_plan(&decoder, 2)
...
fn adapter_device_plan_rejects_non_eoi_marker_after_entropy() {
    ...
    let decoder = Decoder::new(&bytes).expect("restart-coded decoder");
    let err = ashlar_jpeg::adapter::build_device_plan(&decoder, 2)
        .expect_err("unexpected marker should fail");

    assert!(matches!(
        err,
        ashlar_jpeg::JpegError::UnexpectedMarker {
            expected: ashlar_jpeg::MarkerKind::Eoi,
            found: 0xe0,
            ..
        }
    ));
