...
struct ContextInner {
    ...
    nvjpeg_batch: Mutex<Option<nvjpeg::NvjpegState>>,
}
...
impl Drop for ContextInner {
    fn drop(&mut self) {
        if !self.context.is_null() {
            ...
            drop(nvjpeg.take());
            let nvjpeg_batch = match self.nvjpeg_batch.get_mut() {
                Ok(nvjpeg) => nvjpeg,
                Err(poisoned) => poisoned.into_inner(),
            };
            drop(nvjpeg_batch.take());
...
impl CudaContext {
    ...
    pub fn decode_jpeg_rgb8_batch_with_nvjpeg(
        ...
        let mut device_ptrs = Vec::with_capacity(inputs.len());
        let mut pitches = Vec::with_capacity(inputs.len());
        for (_bytes, dimensions) in inputs {
            let (pitch_bytes, byte_len) = rgb8_layout(*dimensions)?;
            let buffer = self.allocate(byte_len)?;
            device_ptrs.push(buffer.device_ptr());
        ...
        if state.is_none() {
            *state = Some(nvjpeg::NvjpegState::new_batched()?);
        }
        let state = state.as_mut().ok_or_else(|| CudaError::NvjpegUnavailable {
            message: "nvJPEG batch state did not initialize".to_string(),
        })?;
        state.decode_rgb8_batch(inputs, &device_ptrs, &pitches)?;
