...
fn scalar(input: &[i16; 64]) -> [u8; 64] {
    let mut out = [0u8; 64];
    signinum_jpeg::bench_support::bench_idct_reference_block_with(input, &mut out);
    out
}
...
fn assert_neon_matches_scalar(input: &[i16; 64]) {
    ...
    signinum_jpeg::bench_support::bench_idct_neon_block(input, &mut neon_out);
    assert_eq!(
        scalar_out, neon_out,
...
fn assert_avx2_matches_scalar(input: &[i16; 64]) {
    ...
    signinum_jpeg::bench_support::bench_idct_avx2_block(input, &mut avx_out);
    assert_eq!(
        scalar_out, avx_out,
