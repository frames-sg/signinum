...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=SLIDECODEC_GROK_ROOT");
    println!("cargo:rerun-if-env-changed=SLIDECODEC_GROK_SOURCE");
    ...
    if let Some(config) = grok_config() {
        ...
        cc::Build::new()
            ...
            .warnings(false)
            .compile("slidecodec_j2k_grok_shim");
    }
}
...
fn grok_config() -> Option<GrokConfig> {
    let source_root = std::env::var_os("SLIDECODEC_GROK_SOURCE")
        .map_or_else(|| PathBuf::from("/tmp/grok-slidecodec"), PathBuf::from);
    let lib_dir = std::env::var_os("SLIDECODEC_GROK_ROOT")
