...
impl OpenJpegPaths {
    fn discover() -> Option<Self> {
        let compress = std::env::var_os("SLIDECODEC_OPENJPEG_COMPRESS_BIN")
            .map(PathBuf::from)
            .or_else(|| Some(PathBuf::from("/opt/homebrew/bin/opj_compress")))
            .filter(|path| path.exists())?;
        let decompress = std::env::var_os("SLIDECODEC_OPENJPEG_BIN")
            .map(PathBuf::from)
...
fn temp_dir() -> &'static Path {
    ...
    DIR.get_or_init(|| {
        let dir =
            std::env::temp_dir().join(format!("slidecodec-j2k-parity-{}", std::process::id()));
