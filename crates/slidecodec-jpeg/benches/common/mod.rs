...
/// decoder; fairness is preserved by keeping this group slidecodec-only.
pub(crate) fn slidecodec_decode_reused(
    dec: &Decoder<'_>,
    out: &mut [u8],
    stride: usize,
    fmt: PixelFormat,
...
pub(crate) fn slidecodec_decode_with_scratch(
    dec: &Decoder<'_>,
...
pub(crate) fn slidecodec_decode_rows(bytes: &[u8]) {
    let dec = Decoder::new(bytes).expect("slidecodec decoder");
    let mut sink = NullSink;
    dec.decode_rows(&mut sink).expect("slidecodec decode_rows");
}

pub(crate) fn slidecodec_decode_tile_batch(bytes: &[u8], batch_size: usize) {
    let mut ctx = DecoderContext::new();
    let mut pool = ScratchPool::new();
    let mut sink = NullSink;
    for _ in 0..batch_size {
        Decoder::decode_tile(bytes, &mut ctx, &mut pool, &mut sink)
            .expect("slidecodec decode_tile batch");
    }
}
...
pub(crate) fn slidecodec_decode_tile_batch_region_scaled(
    bytes: &[u8],
    batch_size: usize,
    side: u32,
    factor: Downscale,
...
pub(crate) fn slidecodec_decode_region(bytes: &[u8], side: u32) {
    let dec = Decoder::new(bytes).expect("slidecodec decoder");
