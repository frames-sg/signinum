...
use common::{
    ...
    jpeg_decoder_inspect, load_bench_inputs, scaled_rect, slidecodec_decode,
    slidecodec_decode_region, slidecodec_decode_region_scaled, slidecodec_decode_rows,
    slidecodec_decode_scaled, slidecodec_decode_tile_batch_region_scaled,
    slidecodec_decode_tile_batch_scaled, slidecodec_inspect, zune_decode,
...
fn main() {
    ...
    if std::env::var_os("SLIDECODEC_BENCH_INPUTS").is_some() {
    ...
    let iterations = std::env::var("SLIDECODEC_REPORT_ITERS")
        .ok()
...
fn run_input(input: &BenchInput, iterations: usize) -> Vec<ReportRow> {
    ...
    match (input.mode, input.input_class) {
            ...
            if should_compare_full_frame(input.mode, input.input_class) =>
        {
            ...
            rows.push(run_slidecodec_only_row(
                input,
                Operation::DecodeRowsRgb,
                iterations_for(input, Operation::DecodeRowsRgb, iterations),
                "comparator skipped for very large RGB input; report uses slidecodec decode_rows",
            ));
        }
        (DecodeMode::Rgb, CorpusInputClass::VeryLarge) => {
            rows.push(run_slidecodec_only_row(
...
fn run_compare_row(input: &BenchInput, operation: Operation, iterations: usize) -> ReportRow {
    let (slidecodec, jpeg_decoder, zune) = run_compare_measurements(operation, input, iterations);
    ReportRow {
...
fn run_slidecodec_only_row(
    ...
    ReportRow {
        ...
        input_class: input.input_class,
        operation,
        slidecodec: run_measurement(Library::Slidecodec, operation, input, iterations),
...
fn is_supported(library: Library, operation: Operation, input: &BenchInput) -> bool {
    matches!(
        ...
        (Library::Slidecodec, Operation::Inspect, _, _)
...
fn run_compare_measurements(
    ...
    let mut slidecodec = MeasurementState::new(
        Library::Slidecodec,
        is_supported(Library::Slidecodec, operation, input),
        iterations,
    );
    ...
    let mut states = [&mut slidecodec, &mut jpeg_decoder, &mut zune];
    ...
    (slidecodec.finish(), jpeg_decoder.finish(), zune.finish())
}
...
fn run_operation(library: Library, operation: Operation, input: &BenchInput) {
    match (library, operation) {
        (Library::Slidecodec, Operation::Inspect) => slidecodec_inspect(&input.bytes),
        ...
        (Library::Slidecodec, Operation::DecodeRgb) => {
            slidecodec_decode(&input.bytes, DecodeMode::Rgb)
        }
        ...
        (Library::Slidecodec, Operation::DecodeGray) => {
            slidecodec_decode(&input.bytes, DecodeMode::Gray)
        }
        ...
        (Library::Slidecodec, Operation::DecodeRowsRgb) => slidecodec_decode_rows(&input.bytes),
        (Library::Slidecodec, Operation::WsiRegionRgb) => {
            slidecodec_decode_region(&input.bytes, ROI_SIDE)
        }
...
fn render_csv(rows: &[ReportRow]) -> String {
    let mut csv = String::from(
        "input,mode,class,operation,slidecodec_ns,jpeg_decoder_ns,zune_ns,slidecodec_error,jpeg_decoder_error,zune_error,fastest\n",
    );
    for row in rows {
        ...
        csv.push_str(&format!(
            ...
            render_ns(&row.slidecodec),
            render_ns(&row.jpeg_decoder),
            render_ns(&row.zune),
            escape_csv(row.slidecodec.error.as_deref().unwrap_or("")),
...
fn render_markdown(rows: &[ReportRow], iterations: usize) -> String {
    ...
    md.push_str("# Slidecodec JPEG corpus report\n\n");
    md.push_str(&format!(
        "- inputs: {}\n",
        rows.iter()
    ...
    md.push_str("| operation | slidecodec fastest | vs jpeg wins | vs jpeg losses | vs zune wins | vs zune losses | failures |\n");
    md.push_str("|---|---:|---:|---:|---:|---:|---:|\n");
    for (operation, stats) in &summary {
        md.push_str(&format!(
            "| {} | {} | {} | {} | {} | {} | {} |\n",
            operation,
            stats.slidecodec_fastest,
            stats.vs_jpeg_wins,
    ...
    md.push_str("\n## Rows where slidecodec is not fastest\n\n");
    md.push_str("| input | operation | slidecodec | jpeg-decoder | zune-jpeg | fastest |\n");
    md.push_str("|---|---|---:|---:|---:|---|\n");
    let mut any_slower = false;
    for row in rows {
        let fastest = fastest_label(row);
        if fastest == Some("slidecodec") || fastest.is_none() {
            continue;
        }
        any_slower = true;
        md.push_str(&format!(
            ...
            row.input_name,
            row.operation.as_str(),
            format_measurement(&row.slidecodec),
    ...
    md.push_str("\n## Failures / skips\n\n");
    md.push_str("| input | operation | slidecodec | jpeg-decoder | zune-jpeg |\n");
    md.push_str("|---|---|---|---|---|\n");
    let mut any_failures = false;
    for row in rows {
        if row.slidecodec.error.is_none()
        ...
        md.push_str(&format!(
            ...
            row.input_name,
            row.operation.as_str(),
            row.slidecodec.error.as_deref().unwrap_or("ok"),
...
impl Summary {
    fn accumulate(&mut self, row: &ReportRow) {
        if fastest_label(row) == Some("slidecodec") {
            self.slidecodec_fastest += 1;
        }
        match compare_measurements(&row.slidecodec, &row.jpeg_decoder) {
            Some(Ordering::Less) => self.vs_jpeg_wins += 1,
            Some(Ordering::Greater) => self.vs_jpeg_losses += 1,
            _ => {}
        }
        match compare_measurements(&row.slidecodec, &row.zune) {
            Some(Ordering::Less) => self.vs_zune_wins += 1,
            Some(Ordering::Greater) => self.vs_zune_losses += 1,
            _ => {}
        }
        if row.slidecodec.error.is_some()
...
fn fastest_label(row: &ReportRow) -> Option<&'static str> {
    ...
    for measurement in [&row.slidecodec, &row.jpeg_decoder, &row.zune] {
